...
aggVariables = ["PositionRep", "CashflowRep"]
origin = ""
...
#Mapped Values are routes or not
isRoute = input("¿Los valores mapeados son rutas? Y o N \n")
if(isRoute=='Y'):
    isRoute = True
else:
    isRoute = False
...
dictRepositories = {"CBC_CLF":"ProductTypeCBC", "REPLNET":"NettingTot", "POS_STG":"PositionStaging", "PROD_CLF":"ProductType", "Aggregation":"Aggregation", "CPARTY_CLF":"CounterpartyType", "FUND_CLF":"ProductTypeFunding", "NET_CLF":"Netting", "ult_parent_cparty": ""}
...
def newAggregationVarSheet(name):
    workAggregationSheet = workbook.add_worksheet(name)
    workAggregationSheet.write(0, 0, "Variable", header_format)
    workAggregationSheet.write(0, 1, "Origen", header_format)
    workAggregationSheet.write(0, 2, "Expression", header_format)
    workAggregationSheet.write(0, 3, "Variable", header_format)
    workAggregationSheet.write(0, 4, "Origen", header_format)
    workAggregationSheet.write(0, 5, "Dependencias", header_format)
    indexSheet = 6
    for inform in portfDictionaries:
        workAggregationSheet.write(0, indexSheet, "Está en " + inform["name"], header_format) 
        indexSheet = indexSheet + 1
...
#Get the final dependencies from Calc_Input if it exists
...
#Enter into the aggregation to analize it
def enterAggregation(jsonArchive, workSheetParam, aggregationName):
    global axiomVarReport
    varExpression = ""
    varName = ""
    ...
    for nodes in jsonArchive["nodes"]["parameterNode"]["parameterNode"][1]["parameterNode"]:
        if (isinstance(nodes["parameters"]["param"]["paramLine"], list)):
            for paramLine in nodes["parameters"]["param"]["paramLine"]:
                workSheetParam.write(index, 0, nodes["_name"])
                listOfModels = analyzeOrigin(paramLine["param"][0]["string"])
                workSheetParam.write(index, 1, paramLine["param"][0]["string"])
                workSheetParam.write(index, 2, paramLine["param"][1]["string"])
                workSheetParam.write(index, 3, " ".join(getDependencies(paramLine["param"][1]["string"])[0]))
                workSheetParam.write(index, 4, " ".join(getVariableOrigin(paramLine["param"][1]["string"])))
                workSheetParam.write(index, 5, " ".join(getDependencies(paramLine["param"][1]["string"])[1]))
                #CheckValueInReport
                dependencies = getDependencies(paramLine["param"][1]["string"])
                for n,model in enumerate(dependencies[0]):
                    axiomVarReport[dependencies[1][n]] = {"models" : [], "dependencies" : [], "parent": ""}
                    dependenciesSplitted = model.split("$Portfolio")
                    if(len(dependenciesSplitted)> 1):
                        dependency = dependenciesSplitted[1]
                        axiomVarReport[dependencies[1][n]]["dependencies"] = axiomVarReport[dependencies[1][n]]["dependencies"] + getDependenciesOfCalc(dependencies[1][n], dependency)
                    else:
                        print(model)
        else:
            workSheetParam.write(index, 0, nodes["_name"])
            listOfModels = analyzeOrigin(nodes["parameters"]["param"]["paramLine"]["param"][0]["string"])
            workSheetParam.write(index, 1, nodes["parameters"]["param"]["paramLine"]["param"][0]["string"])
            workSheetParam.write(index, 2, nodes["parameters"]["param"]["paramLine"]["param"][1]["string"])
            workSheetParam.write(index, 3, " ".join(getDependencies(nodes["parameters"]["param"]["paramLine"]["param"][1]["string"])[0]))
            workSheetParam.write(index, 4, " ".join(getVariableOrigin(nodes["parameters"]["param"]["paramLine"]["param"][1]["string"])))
            workSheetParam.write(index, 5, " ".join(getDependencies(nodes["parameters"]["param"]["paramLine"]["param"][1]["string"])[1]))
            #CheckValueInReport
            dependencies = getDependencies(nodes["parameters"]["param"]["paramLine"]["param"][1]["string"])
            for n,model in enumerate(dependencies[0]):
                    axiomVarReport[dependencies[1][n]] = {"models" : [], "dependencies" : [], "parent": ""}
    ...
    workSheetParam.autofilter(0, 0, index, (8 + len(portfDictionaries)))
    setColumnsWidth(workSheetParam)
    axiomVar[aggregationName] = axiomVarReport.copy()
...
def getVariableFromCube(jsonArchive, variable):
    model = ""
    for nodes in jsonArchive["nodes"]["parameterNode"]["parameterNode"][1]["parameterNode"]:
        if (isinstance(nodes["parameters"]["param"]["paramLine"], list)):
            for paramLine in nodes["parameters"]["param"]["paramLine"]:
                if(variable == nodes["_name"]):
                    if(getVariableOrigin(paramLine["param"][1]["string"])):
                        model = " ".join(getVariableOrigin(paramLine["param"][1]["string"]))
                    else:
                        model = paramLine["param"][1]["string"]
                    break
        else:
            if(variable == nodes["_name"]):
                if(getVariableOrigin(nodes["parameters"]["param"]["paramLine"]["param"][1]["string"])):
                    model = " ".join(getVariableOrigin(nodes["parameters"]["param"]["paramLine"]["param"][1]["string"]))
                else:
                    model = nodes["parameters"]["param"]["paramLine"]["param"][1]["string"]
                break
    #print(model)
    if(model in dictRepositories.keys()):
        model = dictRepositories[model]
    #In case it doesn't have Origin, it returns the variable begining with the word "expression"
    else:
        mode = "expresion: " + model
    return model       
...
def getConditions(registers, value, parent):
    ...
    for register in registers["registers"]:
        if((parent[0] == register["name"]) or not(parent)):
...
def paintConditions(conditions, portfWorkSheet, value, name, origin, index):
    global axiomVarReport
    i = index
    for condition in conditions:
        ...
        for cvalue in condition["values"]:
            ...
            #Get new dependencies of values
            models = getVariableOrigin(cvalue)
            for model in models:
                if(model == "Aggregation"):
                    for variable in getVariable(cvalue):
                        axiomVarReport[variable] = {"models" : [], "dependencies" : [], "parent": value}
                        for portfolio in calcDictionaries:
                            if(getDependenciesOfCalc(variable, "["+portfolio["name"]+"]")):
                                print("coincide")
                                axiomVarReport[variable]["parent"] = value
                                axiomVarReport[variable]["models"] = portfolio["name"]
                                axiomVarReport[variable]["dependencies"] = getDependenciesOfCalc(variable, "["+portfolio["name"]+"]")
                                axiomVarReport[variable]["subdependencies"] = ""
                                break
...
def paintOriginOfValues(portfWorkSheet, var, value, model, parent, name, origin, index):
    global isRoute
    global axiomVarReport
    i = index
    conditions = []
    if(isRoute):
        for portfolio in mappingDictionaries:
            if(searchValues(portfolio, value, parent)):
                ...
                break
        i = paintConditions(conditions, portfWorkSheet, value, name, origin, index)
    else:
        with open("Repository/Cube.json") as jsonCube:
            jsonCubepy = json.load(jsonCube)
            model = getVariableFromCube(jsonCubepy, var)
            #print(var + " : " + model)
            if(model[0:9] == "expression"):
                if(checkValueRepeated(axiomVarReport, value, model.split("expression:")[1])):
                    axiomVarReport[value]["models"].append(model.split("expression:")[1])
                    axiomVarReport[value]["dependencies"].append(model.split("expression:")[1])
                    axiomVarReport[value]["subdependencies"] = []
                portfWorkSheet.write(i, 0, name)
                portfWorkSheet.write(i, 1, origin)  
                portfWorkSheet.write(i, 10, model.split("expression:")[1])
                i = i + 1                
            else:
                for portfolio in mappingDictionaries:
                    if((portfolio["name"] == model) or (model == "aggregation")):
                        conditions = searchValues(portfolio, value, parent)
                        i = paintConditions(conditions, portfWorkSheet, value, name, origin, index)
                if(i == index):
                    i = i + 1
        ...
        if(len(register["conditions"]) > 0):
            ...
            for condition in register["conditions"]:
                ...
                for value in condition["values"]:
                    ...
                    #Get mapping from Portfolio and draw the correspondant conditions
                    if condition["values"]:
                        if(not(valueSplitted in axiomVarReport.keys())):
                            axiomVarReport[valueSplitted] = {"models" : [], "dependencies" : [], "parent": parent}
                        iprevious = i
                        i = paintOriginOfValues(portfWorksheet, condition["var"][0]["name"], valueSplitted, register["models"], axiomVarReport[valueSplitted]["parent"], register["name"], origin ,i)
                    else:
                        i = i + 1
        else:
            i = i + 1
        print(str(i) + ":" + register["name"])
        if(len(register["registers"]) > 0):
            i = paintRegistersOfPortf(portfWorksheet, register["registers"], register["name"], i)
    return i
...
#Get variables from aggregation
for aggVariable in aggVariables:
    print(aggVariable)
    workSheetAggregation = newAggregationVarSheet(aggVariable)
    with open("Repository/" + aggVariable + ".json") as jsonAggregation:
        jsonAggregationpy = json.load(jsonAggregation)
        enterAggregation(jsonAggregationpy, workSheetAggregation, aggVariable)
