...
#Registros nodos
registers = []
aggregationNames = ["Position", "Cashflow", "Liqcombined"]
origin = ""
...
dictRepositories = {"CBC_CLF":"ProductTypeCBC", "REPLNET":"NettingTot", "POS_STG":"PositionStaging", "PROD_CLF":"ProductType", "CPARTY_CLF":"CounterpartyType", "FUND_CLF":"ProductTypeFunding", "NET_CLF":"Netting", "ult_parent_cparty": ""}

#repoDictionaries = portfolioConversion.convertPortfolio("Repository")
...
def createWorkSheet(workSheetName):
    ...
    worksheet.write(0, 7, "dependencias", header_format)
...
def getValueOrigin(expression):
    variables = expression.split("=")
    varResult = []
    for variable in variables:
        if(variable[0] == "'") or (variable[1] == "'"):
            varResult.append(variable[1:-1])
    return varResult
...
def getDependenciesOfCalc(value, aggregationName):
    dependencies = []
    if (searchDependency(aggregationName["registers"], value)):
        dependencies = searchDependency(aggregationName["registers"], value)
    if(dependencies):
...
def getValuesOfVariables(report, value, valueModels, valueOrigin, aggregationName):
    if (value in report.keys()):
        if (not report[value]["dependencies"]):
            report[value]["models"] = "".join(valueModels)
            ...
            print(report[value]["dependencies"])
...
def paintOrigin(portfWorkSheet, var, name, origin, index):
    global isRoute
    global axiomVarReport
    for dependency in calcDictionaries:
        dependencyCalc = getDependenciesOfCalc(var, dependency)
        print(dependencyCalc)
        if(len(dependencyCalc) >= 1):
            axiomVarReport[var]["dependencies"] = dependencyCalc
            portfWorkSheet.write(i, 0, name)
            portfWorkSheet.write(i, 1, origin)
            portfWorkSheet.write(index, 7, " ".join(dependencyCalc))
            break
    return index + 1
...
def paintRegistersOfPortf(portfWorksheet, registers, origin, index):
    ...
    for register in registers:
        ...
        if(len(register["conditions"]) > 0):
            ...
            for condition in register["conditions"]:
                for var in condition["var"]:
                    portfWorksheet.write(i + condition["var"].index(var), 3, var["alias"])#varOrigin
                    portfWorksheet.write(i + condition["var"].index(var), 4, var["name"])#var
                    #Get mapping from Portfolio and draw the correspondant conditions
                    if condition["var"]:
                        if(not(var["name"] in axiomVarReport.keys())):
                            axiomVarReport[var["name"]] = {"models" : "", "dependencies" : [], "parent": var["alias"]}
                            i = paintOrigin(portfWorksheet, var["name"], register["name"], origin ,i)
                    else:
                        i = i + 1
                portfWorksheet.write(i, 5, condition["operator"].replace("<=","minor or equal").replace('=', " equal"))#operation
                for value in condition["values"]:
                    if(value[0] == "$"):
                        for n,eachValue in enumerate(getDependencies(value)[1]):
                            if(n  < len(getVariableOrigin(value))):
                                variableOrigin = getVariableOrigin(value)[n]
                            else:
                                variableOrigin = "Aggregation"
                            portfWorksheet.write(i+n, 3, variableOrigin)
                            portfWorksheet.write(i+n, 4, eachValue)
                            portfWorksheet.write(i+n, 5, "equal")
                            if(n  < len(getValueOrigin(value))):
                                valueOrigin = getValueOrigin(value)[n]
                            else:
                                valueOrigin = ""
                                portfWorksheet.write(i+n,6, valueOrigin)
                            if(not(eachValue in axiomVarReport.keys())):
                                axiomVarReport[eachValue] = {"models" : "", "dependencies" : [], "parent": eachValue}   
                            i = paintOrigin(portfWorksheet, eachValue, register["name"], origin ,i)
                    else:
                        portfWorksheet.write(i, 0, register["name"])
                        portfWorksheet.write(i, 1, origin)
                        portfWorksheet.write(i, 6, value)#value
        else:
            i = i + 1
...
for portfolio in mappingDictionaries:
    ...
    index = paintRegistersOfPortf(worksheet, portfolio["registers"], portfolio["name"], 1)
...
#!!Hacerlo bien
#for repository in mappingDictionaries:
    #worksheet = createWorkSheet(portfolio["name"])
...
def fillSheetVar(worksheetVar):
    ...
    for key in axiomVar.keys():
        for var in axiomVar[key].keys():
            ...
            worksheetVar.write(index, 2, axiomVar[key][var]["parent"])
            if not(isinstance(axiomVar[key][var]["dependencies"], list)):
                worksheetVar.write(index, 3, axiomVar[key][var]["dependencies"])
                index = index + 1
            else:
                for dependency in axiomVar[key][var]["dependencies"]:
                    worksheetVar.write(index, 3, dependency)
                    worksheetVar.write(index, 4, axiomVar[key][var]["models"])
                    index = index + 1
